        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-purple-600 font-semibold uppercase tracking-widest sm:hidden">
                Painel Admin
              </p>
            </div>
          </div>

          {/* Direita: badge admin + logout */}
          <div className="flex items-center gap-2 flex-shrink-0">

            {/* Badge do usuário */}
            {username && (
              <div className="flex items-center gap-2 bg-purple-50 border border-purple-200 px-3 py-1.5 rounded-lg">
                <Shield size={13} className="text-purple-600 flex-shrink-0" />
                <div className="hidden sm:block">
                  <p className="text-xs text-purple-500 font-medium leading-none mb-0.5">Admin</p>
                  <p className="text-sm font-bold text-purple-800 leading-none">{username}</p>
                </div>
                <span className="sm:hidden text-sm font-bold text-purple-800 max-w-[72px] truncate">
                  {username}
                </span>
              </div>
            )}
